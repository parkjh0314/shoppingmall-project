...
$(document).ready(function() {
   ...
   IMP.init('imp16840086');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   ...
   IMP.request_pay({
       ...
       name : '결제테스트(코벤져스)',	 // 코인충전 또는 order 테이블에 들어갈 주문명 혹은 주문 번호. (선택항목)원활한 결제정보 확인을 위해 입력 권장(PG사 마다 차이가 있지만) 16자 이내로 작성하기를 권장
       amount : 100,	  // '${coinmoney}'  결제 금액 number 타입. 필수항목. 
       buyer_email : '${member.email}',  // 구매자 email
       buyer_name : '${member.name}',	  // 구매자 이름 
       buyer_tel : '${member.mobile}',    // 구매자 전화번호 (필수항목)
       buyer_addr : '${shippingAddress.address} - ${shippingAddress.detailAddress} - ${shippingAddress.extraAddress}',  
       buyer_postcode : '${shippingAddress.postcode}',
	   ...
	   */
		...
		if ( rsp.success ) { // PC 데스크탑용
			...
			*/
            ...
            self.close();
			// opener.location.href = "javascript:goFinal("+a+ ","+ b + ","+ c+ ");";
			opener.location.href = "javascript:goFinal();";
            ...
            alert("결제에 실패하였습니다.\n장바구니로 이동합니다.");
            self.close();
            location.href="/Covengers/product/myCart.com";
       }
